...
ids<-read.table(filehead,header=F,stringsAsFactor=F) #skip line number might be differ

varmiss<-read.table(paste0(filehead,".lmiss"),sep="",head=T)
...
write.table(duplicated_variants,"./data/list_duplicated_var_exclude",col.names=F,row.names=F,quote=F,sep="\t")
write.table(bim.1[,c(2,7,3:6)],paste0(filehead,"_edited.bim"),col.names=F,row.names=F,quote=F,sep="\t")
