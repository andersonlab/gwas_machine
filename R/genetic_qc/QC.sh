...
wkdir="/lustre/scratch123/hgi/projects/ibdgwas_bioresource/qz2/pre_imputation_b04b06b15b19"
...
cd ${wkdir}
rawDataDir="/lustre/scratch123/hgi/projects/ibdgwas_bioresource/raw/data_transfer_20220228"

mkdir data
batch=(b04 b06 b08 b09 b10 b12 b15 b17 b18 b19)
for i in ${batch[@]}
do
	mkdir data/${i}
	...
	zcat ${rawDataDir}/v2chip_${i}_calls_1.vcf.gz| grep -E -w ^'[1-9]|1[0-9]|2[0-2]|X|Y|MT|#CHROM' > data/${i}/tmp
...
## 5.2. merge batches together, for known duplicated samples across batches, keep the one with highest call rates
# this scripts is designed for UKIBDBR data. Some patients with duplicated samples, keep the sample with smallest missing rate

batch=(b04 b06 b08 b09 b10 b12 b15 b17 b18 b19)
for i in ${batch[@]}
do
	plink --bfile data/${i}/${i}_ATCG_aligned_nodup --missing --out data/${i}/${i}_ATCG_aligned_nodup
done

rm data/all_batch.imiss
for i in ${batch[@]}
do
	awk '{if(NR!=1) print $1,$2,$6}' data/${i}/${i}_ATCG_aligned_nodup.imiss >> data/all_batch.imiss
done


# three parameters: 1. work_dir; 2. input file name with missing rate info for each samples; 3. output file name contains samples with duplicated removed
/software/R-4.1.0/bin/Rscript scripts/5_rm_dup_ind.R ${wkdir} "all_batch.imiss" "ind_uniq.id"

for i in ${batch[@]}
do
        plink --bfile data/${i}/${i}_ATCG_aligned_nodup --keep data/ind_uniq.id --make-bed --out data/${i}/${i}_ATCG_aligned_nodup_keep
done

rm data/merge.list
for i in ${batch[@]:1}
do
        echo "data/${i}/${i}_ATCG_aligned_nodup_keep.bed data/${i}/${i}_ATCG_aligned_nodup_keep.bim data/${i}/${i}_ATCG_aligned_nodup_keep.fam" >> data/merge.list
done

filename="AllBatch_ATCG_aligned_nodup"
plink --bfile data/b04/b04_ATCG_aligned_nodup_keep --merge-list data/merge.list --make-bed --out data/${filename}


## 5.3. keep samples with phenotype and add gender info
## two parameters: 1. work_dir; 2. prefix of filename

/software/R-4.1.0/bin/Rscript scripts/5_rm_noPheno_add_gender.R ${wkdir} ${filename}

plink --bfile data/AllBatch_ATCG_aligned_nodup --keep data/${filename}_pheno_gender --make-bed --out data/${filename}_pheno

filename=${filename}_pheno
...
for((i=1;i<23;i++))
do
        plink2 --bfile /lustre/scratch123/hgi/projects/ibdgwas_bioresource/qz2/ref/1KG/chr${i} --extract ../${filename}_noHighLD_nosex.prune.in --make-bed --out chr${i}
done
